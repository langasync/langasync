"""Example script demonstrating Anthropic Batch API usage.

Usage:
    # Submit a batch job
    python examples/anthropic_example.py run

    # Fetch results (can be run later, even after process restart)
    python examples/anthropic_example.py fetch
...
async def run():
    ...
    print("\nRun 'python examples/anthropic_example.py fetch' to get results.")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python examples/anthropic_example.py [run|fetch]")
        sys.exit(1)
